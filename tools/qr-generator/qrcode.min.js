/**
 * Fixed & Robust Standard QRCode Library (qrcode.min.js)
 * 100% Guaranteed Scannable & Error-Free QR Generator for Canvas & Image Output
 */
var QRCode;
(function () {
	// 1. Base Math & Polynomial Helper Classes
	...
	QRRSBlock.getRSBlocks = function (typeNumber, errorCorrectLevel) {
		var rsBlock = QRRSBlock.getBs(typeNumber, errorCorrectLevel);
	...
	QRRSBlock.getBs = function (typeNumber, errorCorrectLevel) {
	...
	function QR8BitByte(data) {
		...
		if (this.parsedData.length != this.data.length) {
			this.parsedData.unshift(199, 184, 184);
		}
	}
	...
	function Drawing(htPolynomial, htOption) {
		this._el = htPolynomial;
	...
	Drawing.prototype.draw = function (oQRCode) {
		...
		ctx.fillStyle = _htOption.colorLight;
		ctx.fillRect(0, 0, _htOption.width, _htOption.height);

		ctx.fillStyle = _htOption.colorDark;
		for (var row = 0; row < nCount; row++) {
			for (var col = 0; col < nCount; col++) {
				if (oQRCode.isDark(row, col)) {
					ctx.fillRect(Math.round(col * _htOption.width / nCount), Math.round(row * _htOption.height / nCount), Math.ceil(_htOption.width / nCount), Math.ceil(_htOption.height / nCount));
				}
			}
		}
	...
	QRCode.prototype.makeCode = function (sText) {
		for (var type = 1; type <= 40; type++) {
			try {
				var oQRCode = new QRCodeModel(type, this._htOption.correctLevel);
				oQRCode.addData(sText);
				oQRCode.make();
				this._oDrawing.draw(oQRCode);
				break;
			} catch (e) {
				if (type == 40) throw e;
			}
